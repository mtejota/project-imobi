  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: '#f8fafc' }}>
      <div style={{ maxWidth: 1020, margin: '0 auto', padding: '24px 24px 32px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #f1f5f9', borderRadius: 16, padding: 22, marginBottom: 16 }}>
          <div style={{ border: '2px dashed #bfdbfe', borderRadius: 14, background: '#f8fbff', padding: '34px 20px', textAlign: 'center' }}>
            ...
            <button onClick={handleMockUpload} style={{ border: 'none', background: '#1a56db', color: '#fff', borderRadius: 10, padding: '10px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              Selecionar arquivos
            </button>
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #f1f5f9', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 120px 140px', padding: '12px 18px', background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
          ...
          {files.map((file, idx) => (
            <div key={`${file.name}-${idx}`} style={{ display: 'grid', gridTemplateColumns: '2fr 120px 140px', padding: '12px 18px', borderBottom: idx < files.length - 1 ? '1px solid #f8fafc' : 'none', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon d={icons.file} size={15} stroke="#1d4ed8" />
                </div>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#0f172a' }}>{file.name}</span>
              </div>
              <span style={{ fontSize: 11, color: '#64748b', fontFamily: "'DM Mono', monospace" }}>{file.size}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: file.status === 'Concluído' ? '#10b981' : '#f59e0b' }}>{file.status}</span>
            </div>
          ))}
        </div>
      </div>
